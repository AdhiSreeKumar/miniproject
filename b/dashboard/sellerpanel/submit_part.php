...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    // Handle file upload
        ...
        if (!isset($_FILES['part_image']) || $_FILES['part_image']['error'] === UPLOAD_ERR_NO_FILE) {
            $error = "Please select an image file";
        } 
        // Handle other upload errors
        elseif ($_FILES['part_image']['error'] !== UPLOAD_ERR_OK) {
            $errorCode = $_FILES['part_image']['error'];
            $error = "Image upload error: " . ($uploadErrors[$errorCode] ?? "Unknown error (Code: $errorCode)");
        }
        // Process valid upload
            ...
            // Check file size
            if ($_FILES["part_image"]["size"] > $maxFileSize) {
            ...
            // Move uploaded file
            elseif (move_uploaded_file($_FILES["part_image"]["tmp_name"], $targetFile)) {
